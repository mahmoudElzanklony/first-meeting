                        ...
                        if(auth()->check()){
                            if(auth()->user()->activation_phone_status == 1){
                                ...
                                if($sender_country == $receiver_country){
                                    ...
                                    $user->save();
                                    $endpoint = "https://smssmartegypt.com/sms/api/?username=9Wl#7c1&password=zanklony202017&sendername=First Meet&mobiles=20".intval(\App\User::find($receiver_id)->phone)."&message=". urlencode(request('message'))."&Content-Type=application/json;charset=utf-8";
